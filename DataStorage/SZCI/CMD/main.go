...
var (
	SZCIBaseURL 		= "https://api.wmcloud.com/xmlfileread/v1/api/market2/getTickRTSnapshotL2.json?securityID=399001.XSHE&field="
	SZCIToken 			= "Bearer b6275e695f9442f2b08f2f7604e42607e1f9d24157e52d21e0abc9f9947b0bdc"
...
var pullData = func() {
	request, err := http.NewRequest("GET",SZCIBaseURL,nil)
	if err != nil {
		log.Panicf("ERROR----construct request error:%v\n",err)
	}
	request.Header.Set("Authorization",SZCIToken)
	...
	for {
		...
		case <-tick:
			...
			if count == ReconnctMaxTime  {
				...
				log.Printf("ERROR----request szci 3th failed, close szci server----err:%v\n", err)
...
func main() {
	var pricepath= "/root/DataStorage/szciprice.csv"
	...
	var tradepath = "/root/DataStorage/szcitrade.csv"
