...
var (
	USDJPYBaseURL 		= "https://webrates.truefx.com/rates/connect.html?p=anystring&f=csv&s=n&c=USD/JPY"
	...
	for {
		...
		case <-tick:
			...
			if count == ForexReconnctMaxTime {
				log.Printf("ERROR----request usdjpy 3th failed, close usdjpy server----err:%v\n", err)
...
func main() {
	var pricepath= "/root/DataStorage/usdjpyprice.csv"
	...
	var tradepath = "/root/DataStorage/usdjpytrade.csv"
